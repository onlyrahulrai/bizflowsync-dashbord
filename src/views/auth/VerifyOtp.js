...
import { verifyOtp } from 'api/actions/auth';
...
class VerifyOtp extends Component {
  ...
  onResendOTP = () => {
    ...
    this.props.regenerateOtp({ email, mode: "reset" })
      .then((response) => {
        this.setState({
          countDown: 60,
          isResendingOtp: false,
                    ...
                    this.props.verifyOtp(values)
                      .then((response) => {
                        this.setState({ loading: false }, () => {
                          ...
                          this.props.history.push("/auth/change-password", { token: response.data.token });
                        })
                      })
                      .catch((error) => {
                        this.setState({ loading: false }, () => {
                          notification.error("Couldn't verify otp");
                        })
                      })
                  }}
                >
                      ...
                      <form>
                        ...
                        <div className="text-center mt-6">
                          <Button
                            ...
                            disabled={this.state.loading || this.state.isResendingOtp}
...
export default connect(null, { verifyOtp, regenerateOtp })(VerifyOtp);